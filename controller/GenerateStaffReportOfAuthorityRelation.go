...
func GenerateStaffReportOfAuthorityRelation(db *gorm.DB, ansarr map[string]int, staAns model.StaffAnswer) (authorRelRepData []model.StaffDimension, errs error) {
	...
	for key, value := range ansarr {
		...
		if ansort[0].SubjectSort == 354 || ansort[0].SubjectSort == 355 || ansort[0].SubjectSort == 356 || ansort[0].SubjectSort == 357 {
		...
		} else if ansort[0].SubjectSort == 358 {
			/*第五题:面对上级，总是忐忑不安，内心紧张是高焦虑的一种表现；见到上级躲着走是一种高回避的表现。
			根据你和上级大多数在一起的情况，你认为自己属于？*/
			if value == 1130 { /*高焦虑，低回避*/
				if (ansarr["357"] == 1123 || ansarr["357"] == 1124) &&
					(ansarr["356"] == 1119 || ansarr["356"] == 1120 ||
						ansarr["356"] == 1121 || ansarr["356"] == 1122) {
				...
				if (ansarr["356"] == 1119 || ansarr["356"] == 1120) &&
					(ansarr["357"] == 1123 || ansarr["357"] == 1124) {
					if err := db.Raw(`SELECT 
							a.map_id,b.option_name,a.map_explain,a.map_proposal
						FROM
							xy_subject_answer_map_explain a,
			...
			if ansarr["356"] == 1119 && ansarr["357"] == 1125 {
